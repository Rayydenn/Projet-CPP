...
void ApplicGarageWindow::on_pushButtonOpenProject_clicked()
{
    ...
    addAvailableModel(string(m.getName()), m.getBasePrice());
...
void ApplicGarageWindow::on_pushButtonNewProject_clicked()
{
    ...
    for (int i = 0; ; i++)
    {
        Employee e = Garage::getInstance().findEmployeeByIndex(i);
        if (e.getId() == 0) break;
        addTupleTableEmployees(e.tuple());
    }
    ...
    for (int i = 0;; i++)
    {
        Client cl = Garage::getInstance().findClientByIndex(i);
        if (cl.getId() == 0)
            break;


        addTupleTableClients(cl.tuple());
    }
